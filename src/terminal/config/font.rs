...
pub fn true_type<T: AsRef<Path>>(origin: Origin, path: T, tile_size: Size) {}
...
impl BitmapFont {
	builder!(resize, Size);
	builder!(resize_filter, ResizeFilter);
	builder!(resize_mode, ResizeMode);
	builder!(raw_size, Size);
	builder!(codepage, String);
	builder!(align, Align);
	builder!(spacing, Size);
}
...
impl ConfigPart for BitmapFont {
	fn to_config_str(&self) -> String {
		format!("{}: {}{}{}{}{}{}{}{};", self.origin, escape_config_string(&self.path),
			...
			match self.codepage {
				None               => "".to_string(),
				Some(ref codepage) => format!(", codepage={}", codepage),
			},
			match self.align {
				None            => "".to_string(),
...
macro_rules! builder {
	($name:ident, $argt:ty) => {
		pub fn $name(mut self, $name: $argt) -> Self {
			self.$name = Some($name);
			self
		}
	};
